...
if(typeof(getApiData)==undefined){
    let getApiData = document.querySelector('#getApiDataButton');
}
...
async function getApiData(event) {
    ...
    let resp = await fetch(api_url, {method:'GET'});
    let msg = await resp.json();
    mainHandler(msg);
    return false;
}
if(getApiData != null)getApiData.addEventListener('click', getApiData);
...
class HistDataComponent extends Component {
    update(msgR) {
        ...
        let hdata = document.querySelector('#histdatalist');
        if(t != null) {
